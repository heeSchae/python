...
def calculate_score(anwers, correctanw):
    
    point = 0
    
    if not len(anwers) == len(correctanw):
        return -1
    
    else:
        for (key, value) in anwers.items():
            for (question, answer) in correctanw.items():
                
                if key == question and value == correctanw[question]:
                    point += 1
