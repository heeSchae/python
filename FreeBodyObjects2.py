...
def find_net_force(forces):
    ...
    for force in forces:
        Horizontal += force.get_horizontal()
        Vertical += force.get_vertical()
        
    net_magnitude = round((Horizontal **2 + Vertical**2)**0.5, 1)
    net_angle = round(degrees(atan2(Vertical, Horizontal)), 1)
